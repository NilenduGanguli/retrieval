...
export default function SourceCard({ hit, highlighted }: { hit: ChunkHit; highlighted: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Document + page row */}
      <div className="flex items-center gap-1.5 text-[11px] text-zinc-300 mb-2 min-w-0">
      ...
      {Object.keys(hit.score_breakdown).length > 0 && (
        <div className="flex flex-wrap items-center gap-1 mb-2">
          {Object.entries(hit.score_breakdown).map(([k, v]) => (
            <span key={k} className="chip text-[10px] text-muted">
      ...
      <button
        onClick={() => setOpen(o => !o)}
        className="text-left text-xs text-zinc-300 leading-relaxed w-full"
      >
        {open ? hit.content : hit.content.slice(0, 220) + (hit.content.length > 220 ? '…' : '')}
      </button>
      {hit.context && (
        <p className="mt-2 text-[10px] italic text-muted border-l border-line pl-2">
